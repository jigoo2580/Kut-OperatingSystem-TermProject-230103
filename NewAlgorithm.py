...
def round_robin_scheduling(processes, quantum):
    ...
    Btprocess_queue = queue.PriorityQueue()
    # 현재 시간을 0으로 초기화
    current_time = 0
    ...
    for process in processes:
        if process['burst_time'] > quantum:
            Btprocess_queue.put(process)
        else:
            process_queue.put(process)
    Btprocess_queue = sorted(Btprocess_queue, key=lambda x: (x['burst_time'] (reversed=True)))
    # 프로세스 대기열에 첫 번째 프로세스 추가
    # process_queue.put(processes.pop(0))

    # 프로세스 대기열이 비워질 때까지 rr 진행
    while not process_queue.empty() or not Btprocess_queue.empty():
        if not process_queue.empty():
            current_process = process_queue.get()
        else:
            current_process = Btprocess_queue.get()
       ...
       # 프로세스 실행 후 현재 시간 업데이트
        ...
        #     if process['burst_time']>quantum :
        #        Btprocess_queue.put(processes.pop(0))
        #     else :
        #         process_queue.put(processes.pop(0))
        ...
        if current_process['remaining_time'] > 0:
            ...
            for process in list(Btprocess_queue.queue):
                process['waiting_time'] += execution_time  
            if current_process['remaining_time'] <= quantum:
                process_queue.put(current_process)
            else:
                Btprocess_queue.put(current_process)
        else:
